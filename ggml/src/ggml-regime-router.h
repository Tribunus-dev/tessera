...
//
// Regime router: a learned per-(family, shape_bucket) override of
// the static TILE640 cost model in ggml-quants.h
// (ts_t640_outlier_accel_wins / ts_t640_meta_accel_wins).
//
// The static cost model is shape-agnostic: it picks the accel
// path for ALL n_total in (0, 1024] and for ALL n_total_pages >=
// 4096, regardless of which GGUF tensor the call is for. That is
// correct on the M1 base aggregate bench, but per-(family, shape)
// it can be off by 1-2x: an attn_q tensor at 256 elems behaves
// differently from an ffn_down tensor at 256 elems (different
// weight distributions, different outlier ratios, different cache
// residency).
//
// The router adds one layer of per-(family, shape) overrides on
// top of the static threshold. It is a fixed static-include
// lookup table (no runtime allocation, no virtual dispatch, no
// per-call scoring); the calibration runner
// (tools/quantize/tessera/calibrate_regime_router.cpp) produces
// the .gen.h entries, and the dispatch in ggml-ane.mm calls the
// lookup as a static inline. The static cost model is the
// FALLBACK for any (family, shape_bucket) that has no entry: the
// cost model is the contract, the router is the override.
//
// Architecture:
...
//      wins. Otherwise, the helper calls the static
//      ts_t640_*_accel_wins helper and returns its result. The
//      fallback path is the exact same decision the cost model
//      makes on its own; the router NEVER breaks the static
//      contract.
...
//   score_accel = t_l2_accel / mean_latency_accel_us
//   score_scalar = t_l2_scalar / mean_latency_scalar_us
//   winner = (score_accel <= score_scalar) ? accel : scalar
...
// (tools/quantize/tessera/tessera-higgs-proxy.cpp) plus the
// multimodal-only suffixes Tessera cares about. The enum value
// is the policy table key; the string is the human-readable
// label that lands in the .gen.h comments.
//
...
// routes to the static cost model via the lookup miss path.
// ---------------------------------------------------------------------------
...
// (family, shape_bucket) and falls back to the static cost
// model when no entry exists. An empty .gen.h is the baseline:
// every call falls back, and the dispatch behaves exactly as
// the static cost model alone would.
// ---------------------------------------------------------------------------
...
// have seen without the router, and the static cost model is
...
static inline bool ts_regime_router_lookup_outlier(int family,
    ...
    if (family < 0 || family >= TS_REGIME_FAM_COUNT) {
        return ts_t640_outlier_accel_wins(n_total);
    }
    ...
    for (int i = 0; i < kRegimePolicyCount; i++) {
        if (kRegimePolicy[i].family == family &&
            ...
            return kRegimePolicy[i].use_accel_outlier;
        }
    }
    return ts_t640_outlier_accel_wins(n_total);
}
...
static inline bool ts_regime_router_lookup_meta(int family,
                                                ...
                                                int64_t n_pages) {
    if (family < 0 || family >= TS_REGIME_FAM_COUNT) {
        return ts_t640_meta_accel_wins(n_rows, n_pages);
    }
    if (shape_bucket < 0 || shape_bucket >= TS_REGIME_SHAPE_COUNT) {
        return ts_t640_meta_accel_wins(n_rows, n_pages);
    }
    ...
    for (int i = 0; i < kRegimePolicyCount; i++) {
        if (kRegimePolicy[i].family == family &&
            kRegimePolicy[i].shape_bucket == shape_bucket) {
            return kRegimePolicy[i].use_accel_meta;
        }
    }
    return ts_t640_meta_accel_wins(n_rows, n_pages);
}
