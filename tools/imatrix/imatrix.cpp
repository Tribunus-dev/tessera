...
int main(int argc, char ** argv) {
    ...
    if (use_spec) {
        // Wire the imatrix graph-observer lifecycle into the calibration
        // API's observer hooks.  The "begin" hook brackets the verifier's
        // "main" forward (the last of the n_dft+1 per-prefix forwards),
        // and the "flush" hook drains the staging arena after that forward
        // and at the end of the run.  See common/speculative-calibration.h
        // for the exact contract.
        common_speculative_calibration_observer_hooks obs_hooks;
        obs_hooks.begin     = +[](void * ud) { static_cast<IMatrixCollector *>(ud)->begin_graph_observers(); };
        obs_hooks.flush     = +[](void * ud) { return static_cast<IMatrixCollector *>(ud)->flush_graph_observers(); };
        obs_hooks.user_data = &g_collector;
        ...
        opts.observer_hooks = obs_hooks;
        opts.verbosity = 1;

        if (!common_speculative_calibration_run(
                ctx, model, spec.get(), params, n_ctx, opts)) {
