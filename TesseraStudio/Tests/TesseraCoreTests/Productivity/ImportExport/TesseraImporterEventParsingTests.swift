...
final class TesseraImporterEventParsingTests: XCTestCase {
    ...
    func testParseOkEvent() {
        let json = "{\"event\":\"import_ok\",\"path\":\"/tmp/a.docx\",\"format\":\"docx\",\"parser\":\"python-docx\",\"entities\":[{\"entity_id\":\"11111111-1111-1111-1111-111111111111\",\"entity_type\":\"document\"}],\"receipt_ids\":[\"22222222-2222-2222-2222-222222222222\"],\"elapsed_seconds\":0.34}"
    ...
    func testParseFailedEvent() {
        let json = "{\"event\":\"import_failed\",\"path\":\"/tmp/bad.bin\",\"reason\":\"unsupported format\"}"
        let events = TesseraImporter.Event.parseStream(json)
        XCTAssertEqual(events.count, 1)
    ...
    func testParseStream() {
        let json = "{\"event\":\"import_ok\",\"path\":\"/tmp/a\",\"format\":\"md\",\"parser\":\"markdown-it-py\",\"entities\":[],\"receipt_ids\":[],\"elapsed_seconds\":0.1}\n{\"event\":\"import_failed\",\"path\":\"/tmp/b\",\"reason\":\"oops\"}\nnot json\n{\"event\":\"summary\",\"ok\":1,\"failed\":1,\"elapsed\":0.2}"
        let events = TesseraImporter.Event.parseStream(json)
    ...
    func testMalformedUUIDIsSkipped() {
        let json = "{\"event\":\"import_ok\",\"path\":\"/tmp/a\",\"format\":\"md\",\"parser\":\"markdown-it-py\",\"entities\":[{\"entity_id\":\"not-a-uuid\",\"entity_type\":\"document\"}],\"receipt_ids\":[],\"elapsed_seconds\":0.1}"
        let events = TesseraImporter.Event.parseStream(json)
        guard case let .ok(ok) = events[0] else {
            XCTFail("expected .ok")
            return
