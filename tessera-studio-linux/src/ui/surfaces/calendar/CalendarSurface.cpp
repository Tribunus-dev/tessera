...
GtkWidget* calendar_surface_new(ProductivityStore *store){
    ...
    auto nlu_activate = +[](GtkEntry*, gpointer d){ NLUCtx* c=(NLUCtx*)d; const char* raw=gtk_editable_get_text(GTK_EDITABLE(c->entry)); if(!raw||!*raw) return; std::string txt=raw; std::string title=txt, when="today"; if(txt.find("tomorrow")!=std::string::npos) when="tomorrow"; std::string w1=" tomorrow"; auto p1=title.find(w1); if(p1!=std::string::npos) title=title.substr(0,p1); else { std::string w2=" today"; auto p2=title.find(w2); if(p2!=std::string::npos) title=title.substr(0,p2); } GtkWidget* row=gtk_list_box_row_new(); GtkWidget* h=gtk_box_new(GTK_ORIENTATION_VERTICAL,4); GtkWidget* t=gtk_label_new(title.c_str()); gtk_label_set_xalign(GTK_LABEL(t),0); gtk_widget_add_css_class(t,"title-4"); GtkWidget* dt=gtk_label_new((when+" · parsed").c_str()); gtk_label_set_xalign(GTK_LABEL(dt),0); gtk_widget_add_css_class(dt,"dim-label"); gtk_box_append(GTK_BOX(h), t); gtk_box_append(GTK_BOX(h), dt); gtk_list_box_row_set_child(GTK_LIST_BOX_ROW(row), h); gtk_list_box_append(GTK_LIST_BOX(c->list), row); gtk_editable_set_text(GTK_EDITABLE(c->entry),""); };
    g_signal_connect(nluEntry, "activate", G_CALLBACK(nlu_activate), nctx);
